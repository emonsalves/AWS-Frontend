...
const FileInput = ({ darkMode, title, handleFileUpload, error, jsonData, inputRef }) => {
    return (
        <div className={`flex flex-col items-center justify-center p-4 border rounded-lg shadow-md transition-colors ${darkMode
            ? 'bg-gray-800 border-gray-700 text-white'
            : 'bg-white border-gray-300 text-gray-900'
            }`}>
            <h2 className={`text-3xl font-semibold text-center mb-6 ${darkMode ? 'text-white' : 'text-gray-800'}`}>{title}</h2>
            <input
                className={`md:w-11/12 max-w-sm px-4 py-3 border rounded-lg focus:ring-2 focus:ring-[#25D366] focus:border-transparent transition-colors ${darkMode
